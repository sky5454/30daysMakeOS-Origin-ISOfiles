     ...
     struct binding *domainbinding;
{
  ...
  fp = fopen (domain_file->filename, "rb");
  if (fp == NULL)
    return;
  ...
  size = (size_t) st.st_size;
	...
	size = ftell(fp);
	fseek(fp, 0, SEEK_SET);
	if (size < (long) sizeof (struct mo_file_header)) {
		fclose(fp);
		return;
	}

#if defined(HAVE_MMAP) & 0
  ...
  if (data == (struct mo_file_header *) -1)
	...
	return;
      ...
      do
	{
	  long int nb = (long int) fread (read_ptr, 1, to_read, fp);
	  if (nb <= 0)
	    {
#if defined(EINTR) & 0
	      if (nb == -1 && errno == EINTR)
