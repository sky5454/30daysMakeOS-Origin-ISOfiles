     ...
     struct include_file *inc;
{
  ...
  if (/* S_ISREG (inc->st.st_mode) */ 1)
...
#endif
	{
	  ...
	  while (offset < size)
	    {
	      count = fread (buf + offset, 1, size - offset, inc->fp);
#if 0
	      if (count < 0)
		goto perror_fail;
#endif
  ...
  else if (/* S_ISBLK (inc->st.st_mode) */ 0)
    {
  ...
  else
    {
	 ...
	 bigger than the majority of C source files.  */
      ...
      buf = (U_CHAR *) xmalloc (size + 1);
      offset = 0;
      while ((count = fread (buf + offset, 1, size - offset, inc->fp)) > 0)
	{
	  offset += count;
	  if (offset == size)
	    {
